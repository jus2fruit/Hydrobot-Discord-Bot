...
exports.run = async (bot, message, args) => {
  ...
  try {
    const plate = await fsn.readFile('./images/missionpassed.png');
      ...
      avatar = args.join(' ').replace(/gif|webp/g, 'png');
          ...
          Jimp.read(avatar, (err, image) => {
            ...
            Jimp.read(plate, (err, avatar) => {
                avatar.resize(295, 200)
                image.composite(avatar, 4, 60)
                image.getBuffer(Jimp.MIME_PNG, (error, buffer) => {
                  message.channel.send({files: [{ name: 'missionpassed.png', attachment: buffer }] });
                });
            });
        });
